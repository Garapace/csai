    ...
    for i in range(2, int(sqrt(num)) + 1):
        if primes[i]:
            for j in range(i*i, num, i):
                primes[j] = 0
    primes[1] = 0                      # на этом индексе стоит единица, она не является простым числом
    return primes
...
def main():
    while True:
        ...
        try:
            bomba = int(num)
            if bomba > 0:
                primes = [i for i in eratosthenes_algorithm(bomba) if eratosthenes_algorithm(bomba)[i] != 0]
                print(f"простые числа до заданного числа:\n{' '.join(str(item) for item in primes)}"
                      f"\nкол-во простых чисел - {len(primes)}")
                break
            else:
